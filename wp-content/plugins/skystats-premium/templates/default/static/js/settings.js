...
jQuery( window ).ready( function( $ ) {
	...
	$( '#save_settings' ).click( function( e ) {
		...
		$( "input[name='skystats_mashboard_cards_visiblity_status[]']").each( function() {
		...
		$saveSettingsButton.add( $saveSettingsResult ).fadeOut( 400 ).promise().done( function() {
			$loadingImg.fadeIn( 400, function() {
				$.post( ajaxurl, {
					...
					skystats_date_range_label_color: $( '#skystats_date_range_label_color' ).val(),
					...
					skystats_mashboard_menu_name: $( '#skystats_mashboard_menu_name').val(),
	...
	if ( skystats_settings.wp_version >= '3.5' ) {
		...
		$( '#skystats_date_range_label_color' ).wpColorPicker();
	// Fallback to different colorpicker for versions < 3.5.
		...
		$( '#skystats_date_range_label_color').ColorPicker( {
			onChange: function ( hsb, hex, rgb, el ) {
				$( '#skystats_date_range_label_color').val( '#' + hex );
			},
			onSubmit: function ( hsb, hex, rgb, el ) {
				var $el = $( el );
				$el.val( '#' + hex );
