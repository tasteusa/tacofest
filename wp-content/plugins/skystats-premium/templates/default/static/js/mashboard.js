	...
	$.get( ajaxurl, {
		...
		if ( 'error' === response.responseType ) {
			...
			$.get( ajaxurl, {
				...
				var mashboardCardsVisibilityStatus = skystats_mashboard['mashboard_cards_visibility_status'];
				...
				$.each( response, function ( column, cards ) {
					...
					$.each( cards, function ( unused, card ) {
						...
						if ( mashboardCardsVisibilityStatus.hasOwnProperty( card ) && mashboardCardsVisibilityStatus[ card ] == '0' ) {
							$card.hide();
						}
					} );
				} );
				...
				$cardsLoadingContainer.fadeOut( 400, function () {
					...
					var integrations = getIntegrations( 'mashboard', mashboardCardsVisibilityStatus ),
