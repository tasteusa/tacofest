/*http://www.jslint.com/*/
/*jslint white: true */
/*global ajaxurl, skystats_integrations, skystats_mashboard, skystats_google_analytics, skystats_facebook, skystats_twitter, skystats_google_adwords, skystats_mailchimp, console, window*/
...
function GoogleAnalytics( viewName ) {
	...
	this.selectedGoogleAccountEmail = ( 'mashboard' === this.viewName ) ?
		skystats_mashboard['google_analytics']['selected_google_account_email'] :
		skystats_google_analytics['selected_google_account_email'];
	...
	this.$settingsTabChooseGoogleAccountSection = jQuery( '#skystats-google-analytics-settings-choose-google-account-section' );
	this.$settingsTabAddGoogleAccountSection = jQuery( '#skystats-google-analytics-settings-add-google-account-section' );
...
GoogleAnalytics.prototype.getGoogleAccounts = function() {
	...
	jQuery.get(ajaxurl, {
		...
		curInst.$loadingContainer.fadeOut(400, function () {
			if ( 'success' === response['responseType']) {
				var $googleAccountsSelect = jQuery('#skystats-google-analytics-google-accounts'),
				...
				var $saveGoogleAccount = jQuery('#skystats-google-analytics-save-google-account');
				...
				$saveGoogleAccount.one('click', function (e) {
					...
					curInst.$settingsTabSections.fadeOut(400).promise().done(function () {
						curInst.$loadingContainer.fadeIn(400, function () {
							jQuery.post(ajaxurl, {
								action: 'skystats_ajax_google_analytics_save_google_account_email',
								email: email
							}, function () {
								curInst.loadSettingsTabData('cached');
							});
...
GoogleAnalytics.prototype.displaySettingsTabData = function( response ) {
	...
	curInst.$loadingContainer.fadeOut( 400, function () {
		...
		if ( curInst.$settingsTabSetupSection.is( ':visible' ) ) {
			var $authorizeReauthorize = jQuery( '#skystats-google-analytics-authorize' );
		...
		if ( curInst.$settingsTabDeauthorizeSection.is( ':visible' ) ) {
			...
			var $deauthorize = jQuery( '#skystats-google-analytics-deauthorize'),
				$deauthorizeAccountLicenseWide = jQuery( '#skystats-google-analytics-deauthorize-account-license-wide'),
				$deauthorizeAccountsLicenseWide = jQuery( '#skystats-google-analytics-deauthorize-accounts-license-wide');
			// Deauthorize
...
GoogleAnalytics.prototype.registerAuthorizationClickHandler = function() {
	...
	if ( curInst.$settingsTabAddGoogleAccountSection.is(':visible' ) || curInst.$settingsTabReauthorizeSection.is( ':visible' ) ) {
		var $button = jQuery( '#skystats-google-analytics-add-google-account, #skystats-google-analytics-reauthorize' );
		$button.off( 'click' );
		$button.on('click', function( event ) {
			event.preventDefault();
			curInst.selectedGoogleAccountEmail = null;
			jQuery.post( ajaxurl, {
				action: 'skystats_ajax_google_analytics_reset_google_account_email'
			});
			curInst.selectedGoogleAccountEmail = null;
...
function Facebook( viewName ) {
	...
	this.$rangeErrorContainer = jQuery( '#skystats-facebook-range-error-container' );
	this.$rangeErrorMessage = jQuery( '#skystats-facebook-range-error' );
...
function Twitter( viewName ) {
	...
	this.historicalDataErrorContainer = jQuery( '#skystats-twitter-historical-data-notice-container' );
	this.historicalDataError = jQuery( '#skystats-twitter-historical-data-notice' );
...
function GoogleAdwords( viewName ) {
	...
	if ( 'detail' === this.viewName ) {
		...
		this.$campaignLevelAdGroupsTableErrorContainer = jQuery( '#skystats-google-adwords-campaign-level-ad-groups-table' );
		this.$campaignLevelTopKeywordPerformanceTableErrorContainer = jQuery( '#skystats-google-adwords-campaign-level-top-keywords-performance' );
	...
	this.$loadingContainer.fadeOut( 400, function() {
		...
		if ( 'detail' === curInst.viewName ) {
			...
			(function() {
				...
				var $table = jQuery( '#skystats-google-adwords-campaign-level-ad-groups-table' ),
					$columnContent = $table.parent(),
					$column = $columnContent.parent(),
					$tbody = $table.find( 'tbody' ),
			...
			(function() {
				...
				var $table = jQuery( '#skystats-google-adwords-campaign-level-top-keyword-performance-table' ),
					$columnContent = $table.parent(),
					$column = $columnContent.parent(),
					$tbody = $table.find( 'tbody' ),
